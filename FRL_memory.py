...
for model_index in range(model_num):
    ...
    output.append(str(model_index + 1) + 'mutil cla method reconstruction knockoff with prioritized memory replay(half).MEMORY_CAPACITY = 128.BATCH_SIZE = 64.')
    reward_output.append(
        str(model_index + 1) + 'mutil cla method reconstruction knockoff with prioritized memory replay(half).MEMORY_CAPACITY = 128.BATCH_SIZE = 64.')
