...
X_train, X_val, Y_train, Y_val = model_selection.train_test_split(X_original, Y, test_size=0.1, random_state=Random_SEED)
...
class DQN(object):
    ...
    def __init__(self, N_STATES, N_ACTIONS, model_path=None):
        self.eval_net, self.target_net = Net(N_STATES, N_ACTIONS), Net(N_STATES, N_ACTIONS)
        # load saved parameters from pre trained model
...
knockoff_label_y = torch.LongTensor(knockoff_label)
...
dqn = DQN(N_STATES=N_STATES, N_ACTIONS=N_ACTIONS, model_path=current_path)
...
reward_output.append(dataset_name)
...
out = dict(zip(name, output,reward_output))
out = pd.DataFrame([out])
result_folder = './result/knockoff/'
out.to_csv(result_folder + 'pretrain_test.csv', mode='a')
