...
cls_name = ['wine_red', 'wine_white', 'Glycation']
reg_name = ['openml_586', 'openml_589', 'openml_607', 'openml_616', 'openml_618', 'openml_620', 'openml_637', 'airfoil']
...
for dataset_name in name_list:
    ...
    EXPLORE_STEPS = max(N_feature * 10,
                        1000)  # How many exploration steps you'd like, should be larger than MEMORY_CAPACITY20
    VAL_STEPS = N_feature
    N_ACTIONS = 2
    ...
    for model_index in range(model_num):
        ...
        # output.append('random reward')
        # reward_output.append('random reward')
        if dataset_name in reg_name:
            output.append('Original framework with mse, many many models, No ' + str(model_index + 1))
            reward_output.append('Original framework with mse, many many models, No ' + str(model_index + 1))
        elif dataset_name in cls_name:
            output.append('time Original framework with acc, many many models, No ' + str(model_index + 1))
            reward_output.append('Original framework with acc, many many models, No ' + str(model_index + 1))
        ...
        out_1.to_csv(result_folder + 'new_' + dataset_name + '_test_time_greedy.csv', mode='a')
