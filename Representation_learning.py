...
def reconstruction_training(selected, raw):
    ...
    for epoch in range(EPOCH):
        ...
        b_y, encoded, decoded = spp_cnn_autoencoder(raw)
        loss = loss_func(decoded, b_y)      # mean square error
        optimizer.zero_grad()               # clear gradients for this training step
        loss.backward(retain_graph=True)                     # backpropagation, compute gradients
        optimizer.step()                    # apply gradients
        ...
        # print('Epoch: ', epoch, '| train loss: %.4f' % loss.data.numpy())
    selected_b_y, selected_encoded, selected_decoded = spp_cnn_autoencoder(selected)
    # selected_loss = loss_func(selected_decoded, selected_b_y)
    selected_loss = loss_func(selected_decoded, b_y)
    # selected_loss = loss_func(selected_decoded, decoded)
    return selected_loss

def compare_training(selected, raw):
    ...
    for epoch in range(EPOCH):
        ...
        selected_b_y, selected_encoded, selected_decoded = spp_cnn_autoencoder(selected)
        loss = loss_func(selected_decoded, selected_b_y)      # mean square error
        optimizer.zero_grad()               # clear gradients for this training step
        loss.backward(retain_graph=True)                     # backpropagation, compute gradients
        optimizer.step()                    # apply gradients
    ...
    b_y, encoded, decoded = spp_cnn_autoencoder(raw)
    selected_b_y, selected_encoded, selected_decoded = spp_cnn_autoencoder(selected)
    selected_loss = loss_func(selected_encoded, encoded)
    # selected_loss = loss_func(selected_decoded, decoded)
    return selected_loss