...
from sklearn import model_selection, tree, linear_model, svm, neighbors, ensemble
...
knockoff_label = np.load(
    knockoff_label_folder + dataset_name + '_' + measure_type + '_' + threshold_type + '.npy')
#
# # get information
...
MEMORY_CAPACITY = 128  # The size of experience replay buffer
EXPLORE_STEPS = 100 * N_feature  # How many exploration steps you'd like, should be larger than MEMORY_CAPACITY20
VAL_STEPS = N_feature
N_ACTIONS = 2
...
class DQN(object):
    ...
    def learn(self):
        ...
        # ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
        # r_list = self.memory[:, N_STATES + 1:N_STATES + 2].flatten()
        # r_list_clipped = np.clip(r_list, 0, None)
        # r_list_normalized = r_list_clipped / np.sum(r_list_clipped)
        # sample_index = np.random.choice(MEMORY_CAPACITY, BATCH_SIZE, p=r_list_normalized)
        # sample_index = np.random.choice(MEMORY_CAPACITY, BATCH_SIZE)

        r_list = self.memory[:, N_STATES + 1:N_STATES + 2].flatten()
        r_list_clipped = np.clip(r_list, 0, None)
        r_list_normalized = r_list_clipped / np.sum(r_list_clipped)
        sample_index_1 = np.random.choice(MEMORY_CAPACITY, int(BATCH_SIZE / 2), p=r_list_normalized)
        sample_index_2 = np.random.choice(MEMORY_CAPACITY, int(BATCH_SIZE / 2))
        sample_index = np.concatenate((sample_index_1, sample_index_2))
        # ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
...
for i in range(EXPLORE_STEPS):
    ...
    r = (1 - compareLodge_loss.detach().numpy() + knockoff_reward - ave_corr)
    ...
    if (i != 0) and (i % VAL_STEPS == 0):
        for model_index in range(model_num):
            model[model_index].fit(X_train[:, Fstate == 1], Y_train)
            pred_y = model[model_index].predict(X_val[:, Fstate == 1])
...
for model_index in range(model_num):
    ...
    output.append(str(model_index + 1) + 'mutil reg method reconstruction loss with knockoff(half).MEMORY_CAPACITY = 128.BATCH_SIZE = 64.')
    reward_output.append(str(model_index + 1) + 'mutil reg method reconstruction loss with knockoff(half).MEMORY_CAPACITY = 128.BATCH_SIZE = 64.')
